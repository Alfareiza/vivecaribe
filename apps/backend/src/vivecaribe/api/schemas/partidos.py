...
class PartidoShortItem(BaseModel):
    ...
    ``reservas_count`` and ``participants_count`` are computed via LEFT JOIN +
    COUNT/SUM in the repository query (no N+1 queries). Frontend uses these
    to show the badge counts.
    """
